...
namespace Assignment25
{
    public partial class Staff : Form
    {
        ...
        {
            con.Open();
            string sqlQuery = "SELECT TOP 1 ID from Staff order by ID desc";
        ...
        private void DisplayData()
        {
            ...
            adapt = new SqlDataAdapter("select ID,Name,(case when Gender=2 then 'Female' else 'Male' end) Gender, NRC, Address, Phno, Remark from Staff", con);
            adapt.Fill(dt);
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            if (!isUpdate)
            {
                if (txtID.Text != "" && txtName.Text != "" && txtNrc.Text != "" && txtAddress.Text != "" && txtPhno.Text != "")
                {
                    ...
                    cmd = new SqlCommand("insert into Staff(id,name,gender,nrc,address,phno,remark) values(@id,@name,@gender,@nrc,@address,@phno,@remark)", con);
                    con.Open();
            ...
            else
            {
                ...
                if (txtID.Text != "" && txtName.Text != "" && txtNrc.Text != "" && txtAddress.Text != "" && txtPhno.Text != "")
                {
                    cmd = new SqlCommand("update Staff set ID=@id,Name=@name,Gender=@gender,NRC=@nrc,Address=@address,Phno=@phno,Remark=@remark where ID=@id", con);
                    con.Open();
                    cmd.Parameters.AddWithValue("@id", txtID.Text);
                    cmd.Parameters.AddWithValue("@name", txtName.Text);
                    cmd.Parameters.AddWithValue("@gender", gen);
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            if (txtID.Text != "" && txtName.Text != "" && txtNrc.Text != "" && txtAddress.Text != "" && txtPhno.Text != "")
            {
                cmd = new SqlCommand("delete Staff where ID=@id", con);
                con.Open();
                cmd.Parameters.AddWithValue("@id", txtID.Text);
                cmd.ExecuteNonQuery();
                con.Close();
